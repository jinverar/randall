...
- **Register / input control matrix:** rows are Register · Value · Input relationship · Status (`UNKNOWN` / `CORRELATED` / `INFLUENCED` / `CONTROLLED` / `CONFIRMED`). UI displays `CORRELATED` as **Observed Association** (co-occurs — not causation). Built from `InputAttribution` register matches, Influence links, and live Counterfactual deltas. **Never** promote zero/low-value coincidence to `CONFIRMED`. All-ones (`0xFF…FF` / −1) in the payload is at most Observed Association / `CORRELATED` (live CF can raise to `CONTROLLED`, still not `CONFIRMED`).
...
- **Root cause:** null write alone → LOW–MEDIUM leading hypothesis (“NULL/invalid destination reached a write”), not “Parser state error HIGH”. Page Heap detected ≠ UAF. A non-null write AV alone is **not** “attacker-controlled store” — that language requires attribution (pattern / register match / HIGH influence / backward-trace source).
- **Crash intelligence:** `Reproducible` requires the input file on disk; `Minimized` requires a recorded `*_minimized.bin` (not “smallest among cluster files”).
- **Timeline pins:** out-of-window crash bars keep host `Iteration` for sorting but set `CrashIteration` to the real crash iter — UI must not title the host slot as the crash.
