...
- Scream canisters (Crashes tab): harvest rack is **on by default**; **animations off by default** (CPU-safe during fuzz). Toggles on the Crashes harvest panel; prefs in `data/ui-prefs.json` (`screamCanisters`, `screamAnimations`). Art under `docs/assets/canisters/` served as `/canisters/*.jpg`.
