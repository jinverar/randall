...
- Exploit-guided development: `randall exploit guide --exe <p> --core <c> --pattern-len N` builds a tailored next-step playbook (offset → strategy → pwntools) from the crash + `checksec` + ASLR + controlled register/return-address. `randall exploit template …` emits a pwntools skeleton. `exploitdev` also scans the crashed stack at `$rsp` (x86-64 `ret` to a bad address faults at the ret, so the saved return address is on the stack, not RIP). vulnlab `basic`/`nx` tiers are built `-fomit-frame-pointer` for a clean saved-RIP overwrite (offset 72). pwntools installed via `pip install --user --break-system-packages pwntools`; detection is `linux:pwntools` in the doctor. Full roadmap: `docs/EXPLOIT_GUIDE.md`.
