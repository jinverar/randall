...
- Scream canisters (Crashes tab): harvest rack is **on by default**; **animations off by default**. Special seal is **EIP/RIP control** (not fill %). Toggles on the Crashes harvest panel; prefs in `data/ui-prefs.json`. Art under `docs/assets/canisters/` served as `/canisters/*.jpg`.
