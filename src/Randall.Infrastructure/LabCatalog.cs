...
public static class LabCatalog
{
    ...
    [
        ...
        // —— Defense-shaped UAS / sentry labs (fictional UAS1 / STT1 — not real military systems) ——
        new("vulnuas", "VulnUas TCP", "Fictional tactical UAS link (UAS1) — HELLO / ROUTE / SENSOR / TASK length crashes",
            "defense", "intermediate", 15650, "tcp", "randall-vulnuas",
            "targets/vulnuas/randall-vulnuas.exe", "projects/vulnuas.yaml",
            ["defense", "uas", "tcp", "route", "length-field"], DocsPath: "DEFENSE_LAB.md",
            BuildHint: "scripts/build-vulnuas.ps1 · scripts/build-lab-targets.sh",
            ExtraArgs: ["--mode", "tcp"]),
        new("vulnuas-udp", "VulnUas UDP", "Fictional UAS telemetry datagrams (UAS1) — nav / link / bay length crashes",
            "defense", "intermediate", 15651, "udp", "randall-vulnuas",
            "targets/vulnuas/randall-vulnuas.exe", "projects/vulnuas-udp.yaml",
            ["defense", "uas", "udp", "telemetry", "length-field"], DocsPath: "DEFENSE_LAB.md",
            BuildHint: "scripts/build-vulnuas.ps1 · scripts/build-lab-targets.sh",
            ExtraArgs: ["--mode", "udp"]),
        new("vulnturret", "VulnTurret TCP", "Fictional sentry station (STT1) — HELLO / SLEW / TRACK / CONFIG length crashes",
            "defense", "intermediate", 15660, "tcp", "randall-vulnturret",
            "targets/vulnturret/randall-vulnturret.exe", "projects/vulnturret.yaml",
            ["defense", "sentry", "tcp", "track", "length-field"], DocsPath: "DEFENSE_LAB.md",
            BuildHint: "scripts/build-vulnturret.ps1 · scripts/build-lab-targets.sh",
            ExtraArgs: ["--mode", "tcp"]),
        new("vulnturret-udp", "VulnTurret UDP", "Fictional sentry telemetry (STT1) — pose / track / health length crashes",
            "defense", "intermediate", 15661, "udp", "randall-vulnturret",
            "targets/vulnturret/randall-vulnturret.exe", "projects/vulnturret-udp.yaml",
            ["defense", "sentry", "udp", "telemetry", "length-field"], DocsPath: "DEFENSE_LAB.md",
            BuildHint: "scripts/build-vulnturret.ps1 · scripts/build-lab-targets.sh",
            ExtraArgs: ["--mode", "udp"]),
        ...
        new("png-demo", "PNG Demo", "Competitive PNG chunk walker — structured model + intentional lab vulns",
            "file", "intermediate", 0, "file", "",
            "targets/png-demo/app.exe", "projects/png-demo.yaml",
            ["file", "png", "structured-model", "chunk-mutators", "competitive"], Startable: false,
            DocsPath: "FILE_FUZZING.md",
            BuildHint: "scripts/build-png-demo.ps1 · scripts/build-png-demo.sh"),
