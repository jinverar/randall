...
public static class StalkCampaignStore
{
    ...
    public static IReadOnlyList<StalkToolLinkDto> ProbeTools(string? repoRoot = null)
    {
        ...
        [
            ...
            new("sysmon", "Sysmon",
                SysmonEventCapture.IsAvailable(out _, out _) ? "ready" : "missing",
                "Export Operational events for the run window — fuzz.sysmonCapture (service must be pre-installed)",
                "wevtutil epl Microsoft-Windows-Sysmon/Operational sysmon-events.evtx"),
