...
public static class StalkCampaignStore
{
    ...
    public static IReadOnlyList<StalkToolLinkDto> ProbeTools(string? repoRoot = null)
    {
        ...
        [
            ...
            new("tshark", "tshark (pcap)",
                TsharkCapture.DiscoverExecutable(repoRoot) is not null ? "ready" : "missing",
                "Live Wireshark tshark → fuzz.pcapng — fuzz.tsharkCapture (Npcap; often needs elevation)",
                "tshark -i <iface> -w fuzz.pcapng [-f \"host … and port …\"]"),
            new("etw", "ETW / WPR",
                EtwCapture.DiscoverExecutable() is not null ? "ready" : "missing",
                "Light FileIO/Registry/DiskIO/Network ETL — fuzz.etwCapture (open in WPA/PerfView)",
                "wpr -start FileIO.light -start Registry.light -filemode ; wpr -stop fuzz-etw.etl note"),
