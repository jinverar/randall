...
public static class RandallRoadmap
{
    ...
    [
        ...
        new(20, "Protocol packs", "in progress",
        [
            ...
            new("p20-dns", "DNS/mDNS UDP query pack (lab)", false, null),
            new("p20-boofuzz", "Boofuzz example → Scare Floor recipe", false, "scripts/import-boofuzz.py"),
            new("p20-wizard", "Custom protocol wizard (magic + len + body + CRC)", false, null),
        ]),
        new(21, "RPC / DCE-RPC (lab-first)", "planned",
        [
            new("p21-bind", "DCE/RPC bind + request framing model", false, "frag / call_id / opnum"),
            new("p21-ndr", "NDR stub as hex + sized fields (manual layout)", false, null),
            new("p21-session", "Session: bind → request* via sessionFlows", false, null),
            new("p21-lab", "Tiny crashable RPC stub lab server", false, null),
            new("p21-docs", "Docs: fuzz RPC with known stub layout", false, "limits vs Impacket"),
            new("p21-idl", "Optional simple IDL → field map (stretch)", false, null),
        ]),
        new(22, "SMB (session-aware, lab-first)", "planned",
        [
            new("p22-nbss", "NetBIOS session service framing", false, null),
            new("p22-nego", "SMB2 Negotiate + Session Setup (null/guest lab)", false, "no Kerberos v1"),
            new("p22-tree", "Tree Connect + Create + Read/Write models", false, null),
            new("p22-graph", "sessionGraph on SMB status codes", false, null),
            new("p22-lab", "Lab VM guidance for vulnerable SMB", false, "never scan production"),
            new("p22-pipe", "Named pipe → DCERPC reuse (Phase 21)", false, "IPC$"),
        ]),
        new(23, "Layered PDU builder (Scapy workflow, Randall-owned)", "planned",
        [
            new("p23-layers", "Layered PDU builder in Scare Floor", false, "e.g. nbss/smb2"),
            new("p23-fields", "Field table edit (type, endian, fuzzable)", false, null),
            new("p23-templates", "Stack templates from protocol packs", false, null),
            new("p23-json", "Recipe JSON interchange (CLI + UI + scripts)", false, "own format"),
            new("p23-fixups", "Autocalc length/checksum across layers", false, null),
